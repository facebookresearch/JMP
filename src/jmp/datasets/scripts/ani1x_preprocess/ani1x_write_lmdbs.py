...
def write_images_to_lmdb(mp_arg):
    db_path, samples, sampled_ids, idx, pid, uid_mapping, args = mp_arg
    db = lmdb.open(
        db_path,
    ...
    for sysid, fid in samples:
        # Convert from forumula name to a unique id using uid_mapping
        uid = uid_mapping[sysid]
        ...
        txn.commit()
        idx += 1
        sampled_ids.append(f"{sysid},{fid},{uid}" + "\n")
        pbar.update(1)
...
def main(args):
    ...
    # Load the uid mapping:
    # Find all *.traj files in the data path
    data_path = Path(args.data_path)
    uid_mapping = {}
    for traj_file in data_path.glob("*.traj"):
        uid_mapping[traj_file.stem] = len(uid_mapping)
...
def get_parser():
    ...
    parser.add_argument("--data-path", help="Path to dir containing *.traj files")
    parser.add_argument(
        "--out-path",
