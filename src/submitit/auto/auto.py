...
def _convert_deprecated_args(
    kwargs: tp.Dict[str, Any], deprecated_args: tp.Mapping[str, str]
) -> None:
...
class AutoExecutor(Executor):
    ...
    _ctor_deprecated_args = {
        "max_num_timeout": "slurm_max_num_timeout",
        "conda_file": "chronos_conda_file",
    }

    def __init__(
        self, folder: Union[str, Path], cluster: Optional[str] = None, **kwargs: Any
    ) -> None:
        ...
        if self.cluster not in executors:
            raise ValueError(
                f"AutoExecutor doesn't know any executor named {self.cluster}"
            )
    ...
    def which() -> str:
        ...
        if executors[best_ex].affinity() <= 0:
            raise RuntimeError(
                f"Did not found an available executor among {executors.keys()}."
            )
    ...
    def _internal_update_parameters(self, **kwargs: Any) -> None:
        ...
        for ex_arg in specific:
            if len(ex_arg) != 2:
                invalid.append(
                    f"Parameter '{ex_arg[0]}' need to be prefixed by an executor name."
                )
                continue
            ex, arg = ex_arg
        ...
        parameters = self._executor._convert_parameters(
            {k: kwargs[k] for k in kwargs if k in generics}
        )
...
def flexible_init(
    cls: Type[Executor], folder: Union[str, Path], **kwargs: Any
) -> Executor:
    prefix = cls.name() + "_"
    return cls(
        folder,
        **{k[len(prefix) :]: val for k, val in kwargs.items() if k.startswith(prefix)},
    )
