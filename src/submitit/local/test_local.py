...
def test_custom_checkpoint(tmp_path: Path) -> None:
    class Slacker(helpers.Checkpointable):
        ...
        def __submitit_checkpoint__(self, slack: bool = True):
            if slack:
                print(
                    "Interrupted while slacking. I won't slack next time.", flush=True
                )
...
def test_make_subprocess(tmp_path: Path) -> None:
    process = local.start_controller(
        tmp_path,
        "python -c 'import os;print(os.environ[\"SUBMITIT_LOCAL_JOB_ID\"])'",
        timeout_min=1,
    )
    ...
    assert (
        stdout and int(stdout.strip()) == pg
    ), f"PID link is broken (stderr: {stderr})"
