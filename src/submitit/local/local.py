...
class LocalExecutor(core.PicklingExecutor):
    ...
    def _internal_update_parameters(self, **kwargs: Any) -> None:
        ...
        if not isinstance(visible_gpus, Sequence):
            raise ValueError(
                f"Provided visible_gpus={visible_gpus} is not an instance of Sequence."
            )
        if not all(isinstance(x, int) for x in visible_gpus):
            raise ValueError(
                f"Provided visible_gpus={visible_gpus} contains an element that is not an int."
            )
...
class Controller:
    ...
    def _forward_signal(self, signum: signal.Signals, *args: Any) -> None:  # pylint:disable=unused-argument
        for task in self.tasks:
            try:
                task.send_signal(
                    signum
                )  # sending kill signal to make sure everything finishes
            except Exception:
                pass
    ...
    def start_tasks(self) -> None:
        ...
        self.stderrs = (
            ...
            else [p.stderr.open("a") for p in paths]
        )
        for k in range(self.ntasks):
            env = dict(os.environ)
            env.update(
                SUBMITIT_LOCAL_LOCALID=str(k),
                SUBMITIT_LOCAL_GLOBALID=str(k),
                SUBMITIT_LOCAL_JOB_ID=self.pid,
            )
            self.tasks.append(
